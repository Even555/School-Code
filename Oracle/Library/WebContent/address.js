  ...
  $.each(cityJson,
    function(i, val) {
      if (val.item_code.substr(2, 4) == '0000') {
        sb.append("<option id='"+val.item_code+"' value='"+val.item_name+"'>" + val.item_name + "</option>");
      }
    });
...
function doProvAndCityRelation() {
  ...
  if ($("#chooseCity").length === 0) {
    city.append("<option id='chooseCity' value=''>城市</option>");
  }
  if ($("#chooseCounty").length === 0) {
    county.append("<option id='chooseCounty' value=''>区/县</option>");
  }
  var sb = new StringBuffer();
  $.each(cityJson,
    function(i, val) {
      if (val.item_code.substr(0, 2) ==$("#province option:selected").attr("id").substr(0, 2) && val.item_code.substr(2, 4) != '0000' && val.item_code.substr(4, 2) == '00') {
...
function doCityAndCountyRelation() {
  var cityVal = $("#citys option:selected").attr("id");
  var county = $("#county");
  if (county.children().length > 1) {
    county.empty();
  }
